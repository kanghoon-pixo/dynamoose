...
// import {DynamoDB, AWSError} from "@aws-sdk/client-dynamodb";
...
function waitForActive (model: Model<DocumentCarrier>, forceRefreshOnFirstAttempt = true) {
	return (): Promise<void> => new Promise((resolve, reject) => {
		...
		async function check (count: number): Promise<void> {
			...
			const checkSettings: ModelWaitForActiveCheckSettings = typeof model.options.waitForActive === "boolean" ? (originalDefaults.waitForActive as ModelWaitForActiveSettings).check : model.options.waitForActive.check;
			if (count > 0) {
...
export class Model<T extends DocumentCarrier = AnyDocument> {
	...
	async schemaForObject (object: ObjectType): Promise<Schema> {
		const schemaCorrectnessScores: number[] = this.schemas.map((schema) => {
